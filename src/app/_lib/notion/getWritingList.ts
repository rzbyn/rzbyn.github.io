...
/**
 * Fetches a list of writing pages from the Notion database.
 * @returns A promise that resolves to a list of writing items from the Notion database.
 */
export async function getWritingList() {
  const { results } = await getNotionPageList("writing");
  const path = "/writing/";
  return results
